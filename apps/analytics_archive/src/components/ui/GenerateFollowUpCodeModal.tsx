...
import { useCreateFollowUpSession } from "@/hooks/useFollowUpGroups";
import { CreateFollowUpSessionResponse } from "@/services/followUp";
import { DatePicker } from "@workspace/ui/components/date-picker";
import { TimePicker } from "@/components/ui/TimePicker";
import dayjs from "dayjs";

interface GenerateFollowUpCodeModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
  className?: string;
...
export function GenerateFollowUpCodeModal({
  isOpen,
  onClose,
  onSuccess,
  className,
}: GenerateFollowUpCodeModalProps): React.JSX.Element | null {
  const [formData, setFormData] = useState<FollowUpSessionForm>({
    date: "",
    start_time: "",
    end_time: "",
    followup_group_id: "1",
  ...
  const [result, setResult] = useState<Partial<CreateFollowUpSessionResponse>>({});
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (dayjs(selectedDate).isBefore(dayjs(), "day")) {
      setValidationError("Session date cannot be in the past");
      return;
    }
    ...
    try {
      const result = await createMutation.mutateAsync({
        followup_group_id: formData.followup_group_id,
        date: formData.date,
      ...
      setValidationError(
        "Failed to schedule follow-up session. Please try again."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleChange = (field: keyof FollowUpSessionForm, value: string) => {
    setFormData((prev) => ({
      ...prev,
      [field]: value,
    }));
  ...
  const handleCopyCode = async () => {
    if (result && typeof result === "object" && result?.followup_code) {
      try {
        await navigator.clipboard.writeText(result?.followup_code);
        setCopied(true);
          ...
          <div className="flex items-center space-x-3">
            <div>
              <h2 className="text-xl font-semibold text-gray-900">
                Generate Follow-Up Session Code
              </h2>
            </div>
          </div>
          <button
          ...
          <div className="p-8 text-center">
            ...
            <div className="space-y-4">
              <h3 className="text-xl font-bold text-gray-900">
                Follow-Up Session Code Generated
              </h3>
