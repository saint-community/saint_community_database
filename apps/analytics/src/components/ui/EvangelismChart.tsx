...
interface EvangelismChartProps {
  className?: string;
}
...
const chartConfig = {
  value: {
    label: 'Evangelism Sessions',
    color: 'hsl(var(--chart-1))',
  },
};

export function EvangelismChart({ className }: EvangelismChartProps): React.JSX.Element {
  const data = useMemo(() => [
    ...
    <Card className={cn('bg-white', className)}>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle>Evangelism Sessions Analytics</CardTitle>
            <CardDescription>
              Evangelism sessions tracked over the last 5 weeks
            </CardDescription>
          </div>
      ...
      <CardContent>
        ...
        <div className="flex justify-between text-sm text-gray-600 pt-4">
          ...
          <span className="text-blue-600">Trending up 15%</span>
        </div>
      </CardContent>
    </Card>
  );
