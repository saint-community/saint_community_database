...
import { cva, type VariantProps } from "@workspace/ui/lib/utils";
import {
	CheckIcon,
	XCircle,
	ChevronDown,
...
interface MultiSelectProps
	...
	/** The controlled value for selected items. */
	value?: string[];
	...
	(
		...
		const isGroupedOptions = React.useCallback(
			(
				...
				return opts.length > 0 && opts[0] != null && "heading" in opts[0];
			},
			[]
		);
		...
		React.useEffect(() => {
			// Don't reset when component is controlled
			if (value !== undefined || !resetOnDefaultValueChange) return;
		...
		}, [defaultValue, selectedValues, arraysEqual, resetOnDefaultValueChange, value]);
												...
												return (
													<Badge
														...
														<span
															...
															{option.label?.replace(/\s*\(.*/, '').trim() || option.label}
														</span>
														<div
															role="button"
															tabIndex={0}
